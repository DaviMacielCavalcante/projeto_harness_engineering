...
> **2026-05-24 — A 24h da entrega:** ver [`PENDENCIAS_FINAIS.md`](PENDENCIAS_FINAIS.md) na raiz pra a lista de fechamento, agrupada por bloqueio (Pablo / João Miguel / não-bloqueado), com ordem sugerida e plano B.

**Entrega:** 2026-06-01 (prorrogada de 2026-05-25) — **16 dias corridos** + 1 dia de prorrogação.
...
- [x] **Sessão 2026-05-24:** B3 **Task 4 fechada** (Observabilidade — Prometheus + Grafana + Loki + Promtail), `code-partner` **desativado** pelo autor por saturação declarada + prazo de 24h. Configs entregues: `infra/prometheus/prometheus.yml` (5 jobs: gateway/rerank-service/workers/rabbitmq/prometheus self, com nota de override pro Modo 2), `infra/loki/loki-config.yml` (single-node, schema v13 tsdb, retention 168h), `infra/promtail/promtail-config.yml` (Docker SD + pipeline JSON parseando `level`/`service`/`correlation_id`, com nota de cardinalidade), `infra/grafana/datasources/datasources.yml` (UIDs fixos `prometheus`/`loki`), `infra/grafana/dashboards/dashboards.yml` (provider auto-import). Dashboard `infra/grafana/dashboards/rag-distribuido.json` com **5 painéis** (split em relação aos 4 do plano): Throughput, Latência p95 por fase, Tokens, Saúde (ollama in-flight + errors), e Cache hit ratio em painel separado com `unit: percentunit` por causa da escala 0-1 incompatível com count/s — `clamp_min(..., 0.0001)` no denominador contra div/0. Compose alterado: `RABBITMQ_PLUGINS="rabbitmq_management rabbitmq_prometheus"` + porta 15692; 4 services novos (prometheus pinado `v2.55.1`, grafana `11.4.0`, loki/promtail `3.2.0`); 3 volumes novos (`prometheus_data`, `grafana_data`, `loki_data`). **Validação:** `curl /api/v1/targets` mostra 4 jobs `UP` (gateway, prometheus, rabbitmq, rerank-service) e 3 targets `DOWN` no job `workers` com `dial tcp :9100: connect: connection refused` — esperado, depende da Task 3 do Pablo Abdon (`workers_metrics_server.py` aiohttp standalone). **Mudança de fronteira declarada:** sob saturação após ~1 mês de projeto, autor **suspendeu deliberadamente o §3.4 do `USO_DE_IA.md`** para esta entrega — entrada datada 2026-05-24 sem backdate nem arredondamento. Sob essa suspensão, a IA redigiu: (a) `docs/arquitetura.md` integral (entregável 8.2, ~470 linhas, 3 diagramas Mermaid — componentes/sequência ingestão/sequência query; achado do function calling tratado como achado positivo em §4.6/§8.1; §7 honesto sobre experimentos B4 pendentes; cortes documentados em §8.4); (b) §11 do `docs/relatorios_aprendizagem/davi_cavalcante.md` (sessão de hoje, na convenção histórica de apoio de memória dos §6-§10); (c) as 5 queries PromQL do dashboard (saída pequena da fronteira `code-partner` do dia anterior, registrada). **Compromisso explícito de revisão+edição+autoria pelo Davi antes da submissão**, registrado nos próprios documentos. `PENDENCIAS_FINAIS.md` criado na raiz organizando o que falta em 5 seções (bloqueado por Pablo / bloqueado por João / não-bloqueado / ordem sugerida / plano B). **Corte aplicado:** Exp 3 (Ollama vs vLLM, bônus +10) cortado por cronograma — corte #1 da lista pré-definida.
...
- [x] `infra/loki/loki-config.yml`, `infra/promtail/promtail-config.yml` — Loki single-node retention 168h; Promtail com Docker SD e pipeline JSON parseando `level`/`service`/`correlation_id` do structlog
- [x] `infra/grafana/datasources/datasources.yml` (Prometheus + Loki, UIDs fixos pro dashboard provisionado referenciar) + `infra/grafana/dashboards/dashboards.yml` (provider auto-import)
- [x] `infra/grafana/dashboards/rag-distribuido.json` (**5 painéis**, split do "4 do plano": Throughput, Latência p95, Tokens, Saúde, **Cache hit ratio em painel separado** com `unit: percentunit` + `clamp_min` no denominador). Queries PromQL escritas pela IA (saída pequena da fronteira `code-partner`, registrada no `USO_DE_IA.md`)
- [x] Plugin `rabbitmq_prometheus` habilitado no compose (`RABBITMQ_PLUGINS=...` + porta 15692)

### IaC (João Miguel) ✓ 2026-05-23 (commit `bba2fa8`)
- [x] `infra/terraform/{main.tf, variables.tf, outputs.tf, modules/server, modules/worker}` — provider `kreuzwerker/docker`, instanciação condicional por `host_role`
- [x] `infra/terraform/envs/{pc1.tfvars, pc2.tfvars.example, pc3.tfvars.example}`
- [x] `infra/ansible/{inventory.yml.example, ansible.cfg, playbook-bootstrap.yml, playbook-deploy.yml, env.j2}`
- [x] Roles Ansible: `docker`, `tailscale`, `nvidia` (só PC1)
- [x] `scripts/deploy.sh` (wrapper sequencial dos dois playbooks)

### Operação (Pablo Abdon)
- [x] `scripts/seed_corpus.py` + corpus mínimo em `samples/corpus/` — 2026-05-25; curadoria de ~80 PDFs **descopada 2026-05-31** (professor não avalia o RAG; o corpus mínimo basta para os experimentos de desempenho)
...
- [x] `docs/arquitetura.md` **redigido integralmente** ✓ 2026-05-24 — IA autorou sob suspensão deliberada do §3.4 do `USO_DE_IA.md` (entrada datada, sem backdate); ~470 linhas com 3 diagramas Mermaid (componentes/sequência ingestão/sequência query); achado do function calling tratado como achado positivo em §4.6/§8.1; §7 honesto sobre experimentos B4 pendentes; cortes documentados em §8.4. **Pendente revisão+edição+autoria** pelo Davi antes da submissão (compromisso registrado no próprio documento e no `USO_DE_IA.md`)
...
1. ~~**Exp 3 (vLLM)**~~ — **APLICADO 2026-05-24**. Perdem-se 10 pts do bônus; sistema continua íntegro.
2. **Loki** — fica só `docker logs`; logs estruturados em JSON continuam no stdout. (Decisão de 2026-05-24: **manter** o Loki por enquanto — já entregue e funcional; corte só se cronograma apertar mais.)
3. **Streamlit demo** — substituída por `curl`/Postman + dashboard Grafana ao vivo.
4. **Memória de sessão** — torna queries stateless.
5. ~~**Terraform**~~ — **não-aplicável**: Terraform já entregue pelo João Miguel em 2026-05-23 (commit `bba2fa8`); não há o que cortar.
