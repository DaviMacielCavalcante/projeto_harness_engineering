...
## 8. Aprendizados — sessão 2026-05-21 (Task 5 do B2 — ingestão em duas filas)
...
**Fronteira de autoria (§2.5 do `USO_DE_IA.md`):** ao longo desta sessão (Tasks 5, 7 e 8), no modo `code-partner`, a IA entregou estrutura (imports, assinaturas tipadas, docstrings, TODOs) e as suítes de teste como contrato executável, e fez o code-review iterativo; o miolo — a lógica dos handlers, a fórmula do `point_id`, as closures, o método `rerank`, **e a redação dos prompts da Task 8** — foi meu, assim como os bugs, capturados no ciclo de review. A exceção pontual e declarada: na Task 8 deleguei à IA o espelhamento do `system_qa_en.md` (tradução do `pt` que eu redigi) por eficiência, e a IA fez a parte de código (`ContextBlock`). Este §8 é resumo de apoio de memória: eu reviso, ajusto a voz e assumo a autoria.
