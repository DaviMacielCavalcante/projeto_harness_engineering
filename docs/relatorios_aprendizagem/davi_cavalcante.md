...
**Fronteira de autoria (§2.5 do `USO_DE_IA.md`):** nesta sessão, no modo `code-partner`, a IA entregou a estrutura dos três arquivos (imports, assinaturas tipadas, docstrings e TODOs detalhados) e fez o code-review iterativo; o miolo de cada handler — a lógica do loop de chunking, a fórmula do `point_id`, as closures, o roteamento por role — foi meu, e os bugs acima foram meus, capturados no ciclo de review antes do smoke. Este §8 é resumo de apoio de memória: eu reviso, ajusto a voz e assumo a autoria.
