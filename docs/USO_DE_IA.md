...
**Última atualização:** 2026-05-24 (B3/B4 — finalização sob pressão de prazo). A 24h da entrega (25/05) e após cerca de um mês contínuo no projeto, o autor declarou saturação e **suspendeu, deliberadamente e por escrito, a fronteira do §3.4** para esta entrega. Concretamente, autorizou a IA a redigir integralmente:

1. **O documento técnico final (`docs/arquitetura.md`, entregável 8.2)** — primeira vez que isto acontece no projeto. O §3.4 original prevê que a IA forneça "esqueletos" e a equipe redija a versão final; nesta entrega, a IA redigiu prosa, análise e diagramas (Mermaid) **antes** da revisão do autor. O autor revisa, edita e **assume autoria** antes de submeter — mas registra-se que a redação inicial não é dele. Esta entrada **substitui parcialmente** o §3.4: a fronteira ali declarada permanece como princípio do projeto e foi cumprida até 2026-05-23; foi suspensa explicitamente em 2026-05-24 para a entrega final, sob a motivação declarada acima.
2. **As queries PromQL do dashboard Grafana** (`infra/grafana/dashboards/rag-distribuido.json`) — no dia anterior (B3 Task 4), o modo `code-partner` previa que o autor escreveria essas queries (a engenharia de observabilidade real do projeto); o autor desativou o `code-partner` ao iniciar a sessão atual e autorizou a IA a finalizá-las. Mudança pequena de fronteira, mas registrada porque o §2.5 a mencionava como trabalho do autor.
3. **A continuação do `docs/relatorios_aprendizagem/davi_cavalcante.md`** com a sessão de 2026-05-24 — esta parte **não** muda fronteira do §3.4: cai sob a convenção histórica do projeto ("a IA rascunha o resumo do que ele aprendeu; ele revisa, edita e assume a autoria") já praticada nos §6–§10 anteriores. Registrada aqui apenas pra completar o escopo do que a IA produziu nesta sessão.

O que **não** mudou: a apresentação ao vivo (8.3), a defesa em arguição, as decisões técnicas tomadas ao longo do projeto e a validação executada nos hosts reais continuam de responsabilidade do autor e da equipe. A revisão final do documento técnico e do relatório individual antes da submissão é compromisso explícito do autor — sem revisão, o entregável seria ghost-writing, exatamente o que o §4 deste documento rejeita. Esta entrada existe para que, na arguição, o examinador saiba **antes de perguntar** o quanto desta entrega foi mediado pela IA.

Entrada anterior — **2026-05-22** (B2 — fechamento do function calling, req 4.3). O autor conduziu as alavancas de prompt de própria autoria (v0.2.1-b2 imperativo; v0.2.2-b2 remoção do fallback textual) e o miolo de código (`build_messages`/`_render`, integração do preâmbulo de sessão); diante do horário, **delegou explicitamente à IA a autoria do exemplo few-shot do prompt v0.2.3-b2**. Registra-se com franqueza que esta foi a **primeira geração de conteúdo de prompt _original_ pela IA** no projeto — o que **qualifica** a afirmação da entrada de 2026-05-21 ("não houve geração de prompt original pela IA"), verdadeira até então. A delegação foi declarada e ocorreu **após** o autor esgotar suas próprias alavancas de engenharia de contexto; a IA também espelhou os prompts pro `en` e bumpou as versões (mecânico). A decisão de engenharia (aceitar o fallback estrutural e documentar a ressalva de adesão do modelo) e o code-review foram do autor. Entrada anterior — **2026-05-21** (B2 em andamento): o modo `code-partner` (§2.5) seguiu nas Tasks 5, 7 e 8; exceção pontual e declarada na Task 8 (prompts v0.2.0-b2) — o autor redigiu os prompts de próprio punho, mas **delegou à IA o espelhamento** do `system_qa_en.md` a partir do `pt`, e a IA autorou a parte de código associada (`ContextBlock`). Entrada anterior — **2026-05-19**: §2.4, a IA passou a autorar a documentação operacional/de processo, com a fronteira de autoria dos relatórios individuais reafirmada. Este documento será atualizado se o padrão de uso da IA mudar significativamente ao longo do desenvolvimento.
